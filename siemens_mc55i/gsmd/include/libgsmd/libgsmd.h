...
enum lgsm_addr_type {
	...
	LGSM_ATYPE_ISDN_INTL		= 145,
	LGSM_ATYPE_ISDN_NATIONAL	= 129,
};
...
/* initialize usage of libgsmd, obtain handle for other API calls */
extern struct lgsm_handle *lgsm_init(const char *device);
/* use this version for more than one gsmd instance per machine */
extern struct lgsm_handle *lgsm_unit_init(const char *device,
...
extern int lgsm_passthrough_cancel(struct lgsm_handle *lh, int id);
...
extern int lgsm_send_then_free_gmh(struct lgsm_handle *lh, struct gsmd_msg_hdr *gmh);
extern int lgsm_handle_packet(struct lgsm_handle *lh, char *buf, int len);

extern void lgsm_set_checking(struct lgsm_handle *lh, int val);
extern int lgsm_check(struct lgsm_handle *lh);
extern int lgsm_send_ready(struct lgsm_handle *lh);
