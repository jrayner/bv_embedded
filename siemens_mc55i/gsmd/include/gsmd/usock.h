...
/* TODO why are in and out gsmd sizes different? */
...
enum gsmd_msg_phone {
	...
	GSMD_PHONE_POWERSAVE = 3,
	GSMD_PHONE_FLIGHTMODE = 4,
	GSMD_PHONE_GET_MANUF = 5,
	GSMD_PHONE_GET_MODEL = 6,
	GSMD_PHONE_GET_REVISION = 7,
	GSMD_PHONE_GET_IMEI  = 8,
	GSMD_PHONE_SHUTDOWN = 9,
	GSMD_PHONE_SUSPEND = 10,
	GSMD_PHONE_RESUME = 11,
	GSMD_PHONE_AV_CURRENT = 12,
	GSMD_PHONE_VOLTAGE = 13,
};
...
enum gsmd_msg_network {
	...
	GSMD_NETWORK_GET_REG_ST = 14,
};

#define MAX_APN_NAME_LEN 30 // TODO correct value
struct gsmd_gprs_set_config {
	int context;
	char apn[MAX_APN_NAME_LEN];
} __attribute__ ((packed));

enum gsmd_msg_gprs {
	GSMD_GPRS_ATTACH		= 1,
	GSMD_GPRS_DETACH		= 2,
	GSMD_GPRS_ACTIVATE		= 3,
	GSMD_GPRS_DEACTIVATE	= 4,
	GSMD_GPRS_CONNECT		= 5,
	GSMD_GPRS_GET_IP_ADDR	= 6,
	GSMD_GPRS_SET_CONFIG	= 7,
};
...
enum gsmd_ussd_action {
	GSMD_USSD_NO_FURTHER_ACTION	= 0x0,
	GSMD_USSD_ACTION_REQD		= 0x1,
	GSMD_USSD_TERMINATED		= 0x2
};

#define GSMD_USSD_MAXLEN	182
struct gsmd_ussd_notif {
	u_int8_t action_reqd;
	char data[GSMD_USSD_MAXLEN+1];
} __attribute__ ((packed));
...
/* status report can be 167 */
#define GSMD_SMS_DATA_MAXLEN	167
#define GSMD_STATUS_REP_INDEX 0
#define GSMD_STATUS_REP_STATUS 1
struct gsmd_sms {
	u_int8_t physical_byte_length;
	u_int8_t size_encoded_userdata;
	u_int8_t is_statusreport;
	int has_header;
	int is_voicemail;
	struct gsmd_sms_datacodingscheme dcs;
	unsigned char data[GSMD_SMS_DATA_MAXLEN+1];
} __attribute__ ((packed));
...
struct gsmd_test_scenario {
	int type;
	int subtype;
	char scenario[MAX_TEST_NAME_LEN];
} __attribute__ ((packed));
...
enum gsmd_msg_test {
	GSMD_TEST_SET_SCENARIO		= 1,
	GSMD_TEST_SCHEDULE_EVENT	= 2,
};
...
/* Telia has 'Automatisk Kundtj' as one of its names */
/* #define GSMD_PB_TEXT_MAXLEN	14 */
#define GSMD_PB_TEXT_MAXLEN	30
...
#define UCMD_FINAL_CB_FLAG 0x80
...
extern int usock_init(struct gsmd *g, unsigned int instance_num);
extern int usock_cmd_enqueue(struct gsmd_ucmd *ucmd, struct gsmd_user *gu);
...
extern struct gsmd_ucmd *gsmd_ucmd_fill(struct gsmd_user *gu, int len, u_int8_t msg_type,
		u_int8_t msg_subtype, struct gsmd_atcmd *cmd);
extern int valid_ucmd(struct gsmd_ucmd *ucmd);

extern int shutdown_modem(struct gsmd* gsmd, struct gsmd_msg_hdr *gph);
extern void cleanup_sim_busy(struct gsmd *gsmd, int do_cancel_cb);
