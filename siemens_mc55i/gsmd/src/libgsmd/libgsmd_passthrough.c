...
int lgsm_passthrough_cancel(struct lgsm_handle *lh, int id)
{
	return lgsm_cancel(lh, GSMD_MSG_PASSTHROUGH, GSMD_PASSTHROUGH_REQ, id);
}
...
int lgsm_passthrough(struct lgsm_handle *lh, const char *tx, char *rx, unsigned int *rx_len)
{
	...
	rc = lgsm_blocking_wait_packet(lh, rc, rgmh,
