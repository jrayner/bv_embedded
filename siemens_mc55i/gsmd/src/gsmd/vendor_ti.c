...
static int cpmb_detect_cb(struct gsmd_atcmd *cmd, void *ctx, char *resp)
{
	...
	if(rc)
		return gsmd_simplecmd(g, atcmd_buf, PIN_DEPENDENT);
...
static int cpi_detect_cb(struct gsmd_atcmd *cmd, void *ctx, char *resp)
{
	...
	cmd = atcmd_fill("AT%CPMB=1", 10, &cpmb_detect_cb, g, NULL);
	if (cmd) {
        cmd->flags |= ATCMD_PIN_SENSITIVE;
		atcmd_submit(g, cmd, GSMD_CMD_CHANNEL0);
    }

	if (extrsp_supports(er, 0, 3))
		return gsmd_notification_cmd(g, "AT%CPI=3", PIN_DEPENDENT);
	else if (extrsp_supports(er, 0, 2))
		return gsmd_notification_cmd(g, "AT%CPI=2", PIN_DEPENDENT);
	else
...
static int ticalypso_initsettings(struct gsmd *g)
{
	...
	rc |= gsmd_notification_cmd(g, "AT+CTZR=1",NO_PIN_DEPEND);
	/* use %CTZV: Report time and date */
	rc |= gsmd_simplecmd(g, "AT%CTZV=1",NO_PIN_DEPEND);
	/* use %CGREG */
	//rc |= gsmd_simplecmd(g, "AT%CGREG=3");
	/* enable %CPRI: ciphering indications */
	rc |= gsmd_notification_cmd(g, "AT%CPRI=1",NO_PIN_DEPEND);
	/* enable %CSQ: signal quality reports */
	rc |= gsmd_notification_cmd(g, "AT%CSQ=1",NO_PIN_DEPEND);
	/* send unsolicited commands at any time */
	rc |= gsmd_notification_cmd(g, "AT%CUNS=0",NO_PIN_DEPEND);
	/* enable %CPHS: Initialise CPHS Functionalities */
	rc |= gsmd_simplecmd(g, "AT%CPHS=1",NO_PIN_DEPEND);

	/* enable %CPI: call progress indication */
	cmd = atcmd_fill("AT%CPI=?", 9, &cpi_detect_cb, g, NULL);
	if (cmd) {
        cmd->flags |= ATCMD_PIN_SENSITIVE;
		if (g->number_channels > GSMD_NOTIFS_CHANNEL)
			rc = atcmd_submit(g, cmd, GSMD_NOTIFS_CHANNEL);
		else
			rc = atcmd_submit(g, cmd, GSMD_CMD_CHANNEL0);
	}
	return rc;
}
