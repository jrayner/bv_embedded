...
class CommandInterface:
    ...
    def _wait_for_ask(self, info = "", ignorezero=False):
        # wait for ask
        # sometimes we expect to get a spurious 0
        # especially when the bootldr's just been initialised
        got_ack=False
        while got_ack==False:
            try:
                ...
                if ignorezero==True and ask==0x00:
                    continue
                if ask == 0x79:
                    # ACK
                    got_ack=True
    ...
    def sendDummy(self):
        # send a couple of bytes which the boot loader should ignore, but
        # which should wake up the serial port enough so that we can manipulate
        # DTR and RTS. Nasty workaround for unpleasant USB serial adapters.
        # Not really tested.
        try:
            self.sp.write("\x00\xff")
        except:
            pass
    ...
    def reset(self):
        try:
            ...
            mdebug(5, "Warning: setDTR not available, could not set/reset RESET")
    ...
    def initChip(self):
        ...
        try:
            ...
            mdebug(5,"Warning: setRTS not available, could not set BOOT0")
        self.reset()
        mdebug(10,"sending 7F sync byte")
        self.sp.write("\x7F")       # Syncro
        return self._wait_for_ask("Syncro",ignorezero=True)
    ...
    def releaseChip(self):
        try:
            ...
            mdebug(5,"Warning: setRTS not available, could not clear BOOT0")
        self.reset()
